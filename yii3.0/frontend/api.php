...
class Wechat
{
    public function dbs()
    {
        ...
        $db = new PDO($dsn,'jiang', '123456',array(PDO::ATTR_PERSISTENT => true));
        $db->query("set names utf8");
        return $db;
    }
    /**
     * [textinfo 获取粉丝信息-返回设置好的信息]
     * @param  [type] $id [获取粉丝信息]
     * @return [type]     [返回设置好的信息]
     */
    public function textinfo($id) 
    {
        if (isset($id)) {
            $db = $this->dbs();
            $rs = $db->query("select config_token,encodingaeskey,appid,appsecret from wc_account where id = '".$id."' ");
        ...
        switch($type) {
              case Wechat::MSGTYPE_TEXT:
                  $revcon = $weObj->typeText($weObj,$id);
                  $weObj->text($revcon)->reply();
                  exit;
                  break;
              case Wechat::MSGTYPE_EVENT:
                  $weObj->typeEvent($weObj,$id);
                  break;
     ...
     */
    public function typeText($weObj,$id)
    {
         $info = $weObj->returnInfo($weObj,$id);
         if ( $info == false ) {
            return $weObj->returnText($weObj,$id); 
         } else {
            return $info;
         }
    }
   /**
     * [typeEvent 用户欢迎设置]
     * @param  [type] $weObj [类对象]
     * @param  [type] $id    [公众号id]
     * @return [type]        [欢迎语句内容]
     */
    public function typeEvent($weObj,$id)
    {
        ...
        if ($postObj->Event == 'subscribe' ) {
            ...
            //回复欢迎文字消息
            $db = $weObj->dbs();
            $rs = $db->query('select * from wc_userconf where wx_id = "'.$id.' " ');
            $rs->setFetchMode(PDO::FETCH_ASSOC);
            $res = $rs->fetchAll();
            $resinfo = $res[0]['welcome']; 
            $weObj->text($resinfo)->reply();
            // echo "<pre>";
            // var_dump($res);die; 
        }
    }
     ...
     * @return [type]        [description]
     */
    public function returnText($weObj,$id){
        $revcon = $weObj->getRevContent();
        $db = $weObj->dbs();
        $rs = $db->query("select media_id from wc_user_media where wechat_id='$id' and keyword='$revcon'");
        $rs->setFetchMode(PDO::FETCH_ASSOC);
        $res = $rs->fetchAll();
            if ($res)
                { //具体信息
               $rs = $db->query("select * from wc_media where id='".$res[0]['media_id']."' ");
                    $rs->setFetchMode(PDO::FETCH_ASSOC);
                    $resinfo = $rs->fetchAll();
                    return $img=$resinfo[0]["file"];
                } else {
                    return $img = "内容不存在";
                }
            //返回给用户信息
        //$weObj->text($img)->reply(); 
     ...
     * @return [type]        [description]
     */
    public function returnInfo($weObj,$id){
        //查询到用户请求的信息
        $revcon = $weObj->getRevContent();
        $db = $weObj->dbs();

        $rs = $db->query('select apilocal from wc_userapi where wx_id ="'.$id.'" and status =1 and keywords ="'.$revcon.'" ');
        $rs->setFetchMode(PDO::FETCH_ASSOC);

        $result = $rs->fetchAll();
        if (isset($result) && $result !=null) {
            //return $res = file_get_contents($_SERVER['SERVER_NAME'].'/web/index.php?r=setting/'.$result[0]["apilocal"]);  
              //file_put_contents($_SERVER['SERVER_NAME']."/web/test.txt","Hello World. Testing!");
            //return $res = substr($res, 6); 
            //这里是服务器商把  file_get_contents  给关闭了；
            $ch = curl_init();
            $timeout = 5; 
            curl_setopt ($ch, CURLOPT_URL, $_SERVER['SERVER_NAME'].'/web/index.php?r=setting/'.$result[0]["apilocal"]);
            ...
            //return $res =  $_SERVER['SERVER_NAME'].'/web/index.php?r=setting/'.$result[0]["apilocal"];  
        } else {
            return false;
        }
