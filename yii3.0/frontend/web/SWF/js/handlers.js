...
function uploadSuccess(file, serverData) {
	try {
		...
		var status = document.getElementById("dd");
	    status.value = 'D:/phpstudy/WWW/dashixun/yii2/frontend/web/SWF/file/serverData';
	    var fsUploadProgress = document.getElementById("fsUploadProgress");
	    fsUploadProgress.value = '上传成功';
	    // var tishi = document.getElementById("aa");
	    // tishi.innerHTML = "上传成功";
		...
		// alert(serverData);
		 ...
		 // var fsUploadProgress = document.getElementById("fsUploadProgress");
	  //   fsUploadProgress.innerHTML = "上传失败";
		this.debug(ex);
	}
}
...
function uploadError(file, errorCode, message) {
	try {
		...
		case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
			// progress.setStatus("上传错误: " + "上传文件过大或格式只允许是WMA，m4a。或文件重名");
